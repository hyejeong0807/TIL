...
```
`int p[2]`: 파이프의 파일 디스크립터를 저장하는 역할을 합니다. `p[0]`은 파일의 read 디스크립터, `p[1]`은 파이프의 write 디스크립터를 나타냅니다. 

`char *argv[2]`: `exec` 함수에 전달될 명령어와 인자들을 저장합니다. 

`argv[0] = "wc"`: 실행될 프로그램의 이름을 나타냅니다. 

`argv[1] = 0`: 인자가 없음을 나타냅니다. 

`pipe(p)`: `pipe` 함수를 호출하여 파이프를 생성합니다. `p[0]`는 read, `p[1]`는 write 디스크립터 입니다. 

`if (fork() == 0) { ... } else { ... }`: 현재 프로세스를 복제하고 자식 프로세스는 0을 반환하며, 부모 프로세스는 자식 프로세스의 PID를 반환합니다. 이를 이용하여 부모와 자식 프로세스의 동작을 나눠 처리하는 방식입니다. 

#### Child process's block
* `close(0)`: 표준 입력을 닫습니다. 

* `dup(p[0])`: 파이프의 read 디스크립터를 파일 디스크립터 0(STDIN)로 복제합니다. 즉, 표준 입력이 파이프로 변경됩니다. 

* `close(p[0])`: 더 이상 필요 없는 파이프 읽기용 디스크립터를 닫습니다. 

* `close(p[1])`: 파이프 쓰기용 디스크립터를 닫습니다. 

* `exec("/bin/wc", argv)`: "/bin/wc" 프로그램을 실행하고, 이때 인자로 "wc"와 NULL이 전달됩니다. 

#### Parent process's block
* `close(p[0])`: 더 이상 필요 없는 파이프 읽기용 디스크립터를 닫습니다. 
* `write(p[1], "hello world\n", 12)`: 파이프 write 디스크립터를 통해 "hello world\n"을 파이프로 보냅니다. 
* `close(p[1])`: 파이프 write 디스크립터를 닫습니다. 

parent 프로세스가 "hello world\n"을 생성하고 이를 파이프를 통해 child 프로세스로 전송합니다. child 프로세스는 받은 데이터를 표준 입력으로 받아와서 "wc" 프로그램을 실행합니다. 

파이프 read는 데이터가 쓰여질 때까지 또는 write의 끝 지점을 참조하는 모든 파일 디스크립터가 닫힐 때까지 기다립니다. 